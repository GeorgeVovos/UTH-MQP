...
int main()
{
    ...
    #pragma omp parallel for reduction(+ : aValue)
    for (int parallel_i = 0; parallel_i < N; parallel_i++)
        aValue += (A[parallel_i] * A[parallel_i]);

    #pragma omp parallel for reduction(+ : bValue)
    for (int parallel_i = 0; parallel_i < N; parallel_i++)
    ...
    #pragma omp parallel for reduction(+ : eucleidianDistance)
    for (int parallel_i = 0; parallel_i < N; parallel_i++)
        eucleidianDistance += (A[parallel_i] - B[parallel_i]) * (A[parallel_i] - B[parallel_i]);

    #pragma omp parallel for reduction(+ : manhatanDistance)
    for (int parallel_i = 0; parallel_i < N; parallel_i++)
    ...
    #pragma omp parallel for reduction(+ : nominatorSum)
    for (int parallel_i = 0; parallel_i < N; parallel_i++)
        nominatorSum += (A[parallel_i] * B[parallel_i]);

    #pragma omp parallel for reduction(+ : denominatorSum)
    for (int parallel_i = 0; parallel_i < N; parallel_i++)
